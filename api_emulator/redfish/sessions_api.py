...
class SessionCollectionAPI(Resource):
    """
    Session.v1_3_0.SessionCollection
    """

    def __init__(self, rb, suffix):
        """
        SessionCollection Constructor
        """
        self.config = {u'@odata.context': '{rb}$metadata#SessionCollection.SessionCollection'.format(rb=rb),
                       u'@odata.id': '{rb}{suffix}'.format(rb=rb, suffix=suffix),
                       u'@odata.type': u'#SessionCollection.SessionCollection'}
    ...
    def get(self, ident):
        try:
            ...
            self.config['@odata.id']='{prefix}/{ident}/Sessions'.format(prefix=self.config['@odata.id'],ident=ident)
            self.config['Members'] = procs
            self.config['Members@odata.count'] = len(procs)
            resp = self.config, 200
        except Exception as e:
            logging.error(e)
            resp = 'internal error', INTERNAL_SERVER_ERROR
        return resp
